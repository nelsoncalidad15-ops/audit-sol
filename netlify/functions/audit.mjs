...
export default async (request) => {
  ...
  if (!user) return json({ error: 'Acceso no autorizado.' }, 401);

  const scriptUrl = process.env.APPS_SCRIPT_URL;
  const scriptToken = process.env.AUDIT_API_TOKEN;
  if (!scriptUrl || !scriptToken) {
    return json({ error: 'El servicio de auditoría no está configurado.' }, 503);
  }

  try {
    const body = await request.json();
    if (!['save_all', 'upload_evidence'].includes(body.action)) {
    ...
    const response = await fetch(scriptUrl, {
      ...
      body: JSON.stringify({
        ...body,
        token: scriptToken,
        auditorName: user.email || user.user_metadata?.full_name || 'Usuario autorizado',
      }),
    });
